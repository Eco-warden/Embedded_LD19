...
struct FilterParams {
    ...
    // ── 보행자 다리 병합 ────────────────────────────────────────────
    double   merge_radius_mm = 0.0;    // centroid 간 이 거리 이내 클러스터를 병합 (0=비활성)
    double   merge_max_width_ratio = 2.5; // 폭 비율이 이 값 이하인 쌍만 병합 (다리끼리는 OK, 사람+봉투는 NO)
};
    ...
    struct GridKeyHash {
        size_t operator()(const GridKey& k) const {
            return std::hash<int64_t>()(
                (static_cast<int64_t>(k.gx) << 32) | static_cast<uint32_t>(k.gy));
        }
    };
    using GridIndex = std::unordered_map<GridKey, std::vector<size_t>, GridKeyHash>;

    void BuildGrid(const std::vector<CartesianPoint>& points,
